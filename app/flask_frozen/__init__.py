...
class Freezer:
        ...
        with self.app.test_request_context(base_url=script_name or None):
            for generator in url_generators:
                for generated in generator():
                    ...
                    blocked = (
                        (url and fnmatch(url, pattern))
                        or (endpoint and fnmatch(endpoint, pattern))
    ...
    def _check_endpoints(self, seen_endpoints):
        ...
        not_generated_endpoints = {
            endpoint
            for endpoint in (get_endpoints - seen_endpoints)
    ...
    def _build_one(self, url, last_modified=None):
        ...
        with conditional_context(self.url_for_logger, self.log_url_for):
            with conditional_context(
                patch_url_for(self.app), self.app.config["FREEZER_RELATIVE_URLS"]
            ):
                response = client.get(
                    url, follow_redirects=follow_redirects, base_url=base_url
                )
        ...
        if response.status_code != 200:
            if response.status_code == 404 and ignore_404:
                warnings.warn(
                    f"Ignored {response.status!r} on URL {url}",
                    NotFoundWarning,
                    stacklevel=3,
                )
                ...
                raise ValueError(f"Unexpected status {response.status!r} on URL {url}")
    ...
    def make_static_app(self):
        ...
        def dispatch_request():
            ...
            return send_from_directory(self.root, filename, mimetype=guessed_type)
